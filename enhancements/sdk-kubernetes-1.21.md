...
- As always there is risk to the actual development and delivery timing of this feature, due to the many upstream changes that are required to be completed before Operator SDK can support Kubernetes
1.21.

| Repositories where k8s is to bumped to 1.21        | Estimated timeline    |
| -------------------------------------------------- | --------------------- |
| Controller-runtime                                 | Done                  |
| Controller-tools                                   | Done                  |
| Kubebuilder binary, v3 plugin and kb-declarative pattern                   | ~ July 2              |
| Operator-framework/api                             | ~ July 2              |
| Operator-framework/operator-lib                    | ~ July 2              |
| Operator SDK                                       | ~ July 22             |
| Operator-framework/java-operator-plugin            | ~ July 22             |

Given the estimated timeline above, it can be expected that by the third week July, we will have an upstream Operator SDK release which will support Kubernetes 1.21.

- In particular, as the Operator SDK release relates to downstream integrations (e.g. OpenShift), the current OpenShift 4.9 release timelines have feature freeze around September 3. This translates that we would only have around 10 days to cut a [downstream SDK](https://github.com/openshift/ocp-release-operator-sdk) release which supports k8s 1.21.
